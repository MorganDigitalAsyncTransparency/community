...
The backend (Go) has the HTTP API layer (`backend/api/`, `backend/domain/`, `backend/mock/`) implemented with all 17 endpoints from the API contract serving mock data with real domain calculations. The backend serves as the single source of truth for all computation — filtering, bucketing, medians, SLO compliance, and heatmap generation. The data pipeline is implemented and tested: `backend/discourse/` fetches from a Discourse-compatible API, `backend/observer/` normalizes raw topics to domain types, and `backend/storage/` persists to SQLite. A mock Discourse server (`backend/discourse/mockserver/`) enables end-to-end testing without a real forum. The API layer still reads from `backend/mock/`; wiring it to read from SQLite is the next step.
