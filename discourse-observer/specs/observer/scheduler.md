...
SC-5 — **Dev-mode skip.** If Discourse credentials are absent (`DISCOURSE_BASE_URL` is empty), the scheduler does not start. The server still runs for the API. A log message explains why sync is disabled.
