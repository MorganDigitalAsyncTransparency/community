...
2. Read `AI_GUIDELINES.md` in the repository root for module boundaries, scope constraints, and coding expectations.
...
4. Run `make setup` (or `npm run setup` if Make is unavailable) to install dependencies and activate git hooks.
...
2. **Follow the delivery workflow for significant changes.** The phased workflow (requirements → design → validation → implementation → review → PR) is defined in `AGENT.md` in the repository root.
